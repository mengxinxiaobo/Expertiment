...
--------
1. Original PPLAD：
   SKAB 是官方仓库 main.py 的默认数据集，因此 Original 使用
   官方 main.py 默认参数，并自动读取 SKAB 的实际通道数。
2. ASCA-AD / V4：
...
def verify_data(root: Path) -> int:
    ...
    paths = {
        "train": data_dir / "SKAB_train.npy",
        "test": data_dir / "SKAB_test.npy",
        "label": data_dir / "SKAB_test_label.npy",
    }
    for path in paths.values():
        if not path.exists():
            raise FileNotFoundError(f"缺少 SKAB 数据文件：{path}")
    ...
    if train.ndim != 2 or test.ndim != 2:
        raise ValueError(
            f"SKAB 数据应为二维：train={train.shape}, test={test.shape}"
        )
    if train.shape[1] != test.shape[1]:
        raise ValueError("SKAB train/test 通道数不一致。")
    if len(test) != len(labels):
        raise ValueError(
            f"SKAB test 与 label 长度不一致：{len(test)} vs {len(labels)}"
        )

    channels = int(train.shape[1])
    print("SKAB data check passed")
...
def original_config(root: Path, channels: int) -> dict:
    """
    SKAB 是官方仓库 main.py 默认数据集。
    本配置使用官方默认参数，并把 input_c/output_c 设置为实际通道数。
    """
    return {
        "index": 137,
        "dataset": "SKAB",
...
def search_v4_best(
    ...
    """
    SKAB 分数样本较多。每个 score mode 单独收集、评估并释放；
    对全部阈值使用排序+二分查找计算指标，避免逐阈值扫描全量数据。
    """
    print("\nSearching SKAB V4 score modes and thresholds ...")
    ...
    for mode in original_modes:
        ...
        if gt is None:
            raise RuntimeError("未收集到 SKAB 标签。")
    ...
    if best_global is None or best_score_values is None or best_gt is None:
        raise RuntimeError("SKAB 阈值搜索未得到有效结果。")
...
def run_original(Solver, root: Path, channels: int) -> None:
    ...
    output_dir = root / "results" / "SKAB_OFFICIAL_DEFAULT_VS_V4_BEST"
    checkpoint_dir = Path(config["model_save_path"])
    output_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_dir.mkdir(parents=True, exist_ok=True)

    print("================ Original PPLAD / SKAB ================")
    ...
    checkpoint = checkpoint_dir / "SKAB_original_official_default_state.pt"
    ...
    metrics = {
        ...
        "protocol": "official_or_local_SKAB_protocol",
        "config": config,
...
def main() -> None:
    ...
    # 这里提前创建目录，避免 SKAB 在指标已计算完成后因目录不存在而中断。

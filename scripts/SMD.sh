...
CHANNELS=$(python -c "import numpy as np; print(np.load('dataset/SMD/SMD_train.npy', mmap_mode='r').shape[1])")

mkdir -p checkpoints/SMD results/SMD logs/SMD
...
python main.py \
  ...
  --model_save_path checkpoints/SMD \
  --result_path results/SMD \
  2>&1 | tee logs/SMD/smd_v4_seed42.log

...
CHANNELS=$(python -c "import numpy as np; print(np.load('dataset/HAI/HAI_train.npy', mmap_mode='r').shape[1])")

mkdir -p checkpoints/HAI results/HAI logs/HAI
...
python main.py \
  ...
  --model_save_path checkpoints/HAI \
  --result_path results/HAI \
  2>&1 | tee logs/HAI/hai_v4_seed42.log

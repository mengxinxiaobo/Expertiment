...
CHANNELS=$(python -c "import numpy as np; print(np.load('dataset/MSL/MSL_train.npy', mmap_mode='r').shape[1])")

mkdir -p checkpoints/MSL results/MSL logs/MSL
...
python main.py \
  ...
  --model_save_path checkpoints/MSL \
  --result_path results/MSL \
  2>&1 | tee logs/MSL/msl_v4_seed42.log

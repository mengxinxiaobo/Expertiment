...
比较口径
--------
1. Original PPLAD：严格使用官方 scripts/SMD.sh 配置。
...
def verify_data(root: Path) -> int:
    ...
    for path in paths.values():
        if not path.exists():
            raise FileNotFoundError(f"缺少 SMD 数据文件：{path}")
    ...
    if train.ndim != 2 or test.ndim != 2:
        raise ValueError(f"SMD 数据应为二维：train={train.shape}, test={test.shape}")
    if train.shape[1] != test.shape[1]:
        raise ValueError("SMD train/test 通道数不一致。")
    if len(test) != len(labels):
        raise ValueError(f"SMD test 与 label 长度不一致：{len(test)} vs {len(labels)}")

    channels = int(train.shape[1])
    if channels != 38:
        raise ValueError(f"SMD 官方配置应为 38 通道，实际为 {channels}。")
...
def original_config(root: Path, channels: int) -> dict:
    """官方 infogroup502/PPLAD scripts/SMD.sh 配置。"""
    return {
        "index": 137,
        "dataset": "SMD",
        "data_path": "SMD",
        ...
        "loss_fuc": "MSE",
        "anormly_ratio": 0.90,
        "model_save_path": str(root / "checkpoints" / "SMD_OFFICIAL_VS_V4_BEST" / "ORIGINAL"),
        "mode": "train",
        "rec_timeseries": True,
...
def v4_config(root: Path, channels: int, seed: int, epochs: int) -> dict:
    ...
    既往记录中，该训练预算的 PA-F1 高于 win=60/3 epochs 版本。
    ...
    return {
        ...
        "similar": "MSE",
        "rec_timeseries": True,
        "model_save_path": str(root / "checkpoints" / "SMD_OFFICIAL_VS_V4_BEST" / "V4"),
        "result_path": str(root / "results" / "SMD_OFFICIAL_VS_V4_BEST" / "V4"),
        "use_gpu": torch.cuda.is_available(),
        "use_multi_gpu": False,
        "gpu": 0,
        "devices": "0",
...
def search_v4_best(
    ...
    """
    SMD 分数样本较多。每个 score mode 单独收集、评估并释放；
    对全部阈值使用排序+二分查找计算指标，避免逐阈值扫描全量数据。
    """
    print("\nSearching SMD V4 score modes and thresholds ...")
    ...
    for mode in original_modes:
        ...
        if gt is None:
            raise RuntimeError("未收集到 SMD 标签。")
    ...
    if best_global is None or best_score_values is None or best_gt is None:
        raise RuntimeError("SMD 阈值搜索未得到有效结果。")
...
def run_original(Solver, root: Path, channels: int) -> None:
    ...
    output_dir = root / "results" / "SMD_OFFICIAL_VS_V4_BEST"
    checkpoint_dir = Path(config["model_save_path"])
    output_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_dir.mkdir(parents=True, exist_ok=True)

    print("================ Original PPLAD / OFFICIAL SMD ================")
    ...
    checkpoint = checkpoint_dir / "SMD_original_official_state.pt"
    ...
    metrics = {
        ...
        "protocol": "official scripts/SMD.sh",
        "config": config,
...
def main() -> None:
    ...
    # 这里提前创建目录，避免 SMD 在指标已计算完成后因目录不存在而中断。

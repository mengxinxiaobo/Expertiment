...
"""
PSM 上 Original PPLAD 与 V4 best 的统一轻量化基准。
...
------
- 数据集：PSM，25 通道；
- win_size=60，batch_size=256；
- Original：local_size=1，global_size=20，d_model=128；
...
def load_v4_checkpoint(model: V4BestCore, checkpoint: Path, device: torch.device) -> None:
    if not checkpoint.exists():
        raise FileNotFoundError(
            ...
            "请先运行 run_msl_official_vs_v4_best.py --model v4。"
        )
...
def parse_detection_log(model_name: str, root: Path) -> Dict[str, Any]:
    ...
    if model_name == "original":
        path = root / "results/PSM_OFFICIAL_VS_V4_BEST/original_official_metrics.json"
        if not path.exists():
            return result
    ...
    path = root / "results/PSM_OFFICIAL_VS_V4_BEST/v4_best_run.json"
    if not path.exists():
        return result
    data = json.loads(path.read_text(encoding="utf-8"))
    best = data.get("best", {})
...
def benchmark_one(args: argparse.Namespace) -> None:
    ...
    train_file = dataset_root / "PSM/PSM_train.npy"
    test_file = dataset_root / "PSM/PSM_test.npy"
    if not train_file.exists() or not test_file.exists():
        raise FileNotFoundError("找不到 dataset/PSM/PSM_train.npy 或 PSM_test.npy。")

    channels = int(np.load(train_file, mmap_mode="r").shape[1])
    if channels != 25:
        raise ValueError(f"PSM 应为 25 通道，实际为 {channels}。")
    ...
    try:
        ...
        raise RuntimeError("PSM 测试集没有可用 batch。") from exc
...
def aggregate(args: argparse.Namespace) -> None:
    ...
    summary_lines = [
        "# PSM：Original PPLAD（官方配置）vs V4（各自最优结果）",
        "",
        "## 检测精度",
        "",
        ...
        f"| batch=256 平均时延 | {original['single_batch_latency_ms_mean']:.4f} ms | {v4['single_batch_latency_ms_mean']:.4f} ms | {format_reduction(comparison['single_batch_latency_reduction_pct'])} |",
        ...
        "",
        "说明：检测精度部分，Original 使用官方 PSM 配置，V4 使用自身最佳配置及预定义阈值网格中的最高 PA-F1。轻量化部分为受控协议：两种模型统一使用 win_size=60、batch_size=256、相同硬件和相同测试流程。CPU RSS 容易受 Python 分配器影响，应以多次独立进程均值为正式结果。",
    ]
    summary = "\n".join(summary_lines)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/PSM_OFFICIAL_VS_V4_BEST/BENCHMARK",
    )

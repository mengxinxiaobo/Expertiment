...
"""
MSL 上 Original PPLAD 与 V4 best 的统一轻量化基准。
...
class V4BestCore(nn.Module):
    def __init__(self) -> None:
        ...
        # 官方 model 包已经缓存完成。现在必须把官方仓库从 sys.path 首位移除，
        # 否则 `from main import ...` 会错误导入
        # BaselineModels/PPLAD-main/main.py，而不是项目根目录的 V4 main.py。
        if baseline_str in sys.path:
            sys.path.remove(baseline_str)
...
def parse_detection_log(model_name: str, root: Path) -> Dict[str, Any]:
    ...
    if model_name == "original":
        path = root / "results/MSL_OFFICIAL_VS_V4_BEST/original_official_metrics.json"
        if not path.exists():
            return result
    ...
    path = root / "results/MSL_OFFICIAL_VS_V4_BEST/v4_best_run.json"
    if not path.exists():
        return result
    data = json.loads(path.read_text(encoding="utf-8"))
    best = data.get("best", {})
...
def aggregate(args: argparse.Namespace) -> None:
    ...
    summary_lines = [
        "# MSL：Original PPLAD（官方配置）vs V4（各自最优结果）",
        "",
        "## 检测精度",
        "",
        ...
        "",
        "说明：Original 使用官方配置；V4 使用预定义阈值网格中的最高 PA-F1。两者训练轮数可不同；时延和显存使用相同硬件、窗口和 batch 测量。CPU RSS 容易受 Python 分配器影响，应以多次独立进程均值为正式结果。",
    ]
    summary = "\n".join(summary_lines)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/MSL_OFFICIAL_VS_V4_BEST/BENCHMARK",
    )

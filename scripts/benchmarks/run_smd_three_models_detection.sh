...
OUTPUT_ROOT="${PROJECT_ROOT}/results/SMD_PAPER_RESULTS"
...
{
  echo "SMD ASCA-AD V4 vs PPLAD vs LTFAD fixed-protocol detection"
  echo "seed=42"
  echo "standard_scaler=train_fit_only"
  echo "dtype=float32"
  echo "anomaly_ratio=0.9"
  echo "percentile=99.1"
  ...
  echo "PPLAD=fixed_SMD_configuration"
  echo "LTFAD=fixed_fallback_configuration"
  echo "overwrite=${OVERWRITE}"

  "${PYTHON_BIN}" scripts/benchmarks/build_smd_paper_results.py "${ARGS[@]}"
} 2>&1 | tee "${LOG_PATH}"

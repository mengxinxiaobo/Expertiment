...
OUTPUT_ROOT="${PROJECT_ROOT}/results/PUMP_PAPER_RESULTS/Efficiency"
...
{
  echo "PUMP three-model inference efficiency benchmark"
  ...
  echo "full_test_scope=window input -> anomaly score output"
  ...
  "${PYTHON_BIN}" scripts/benchmarks/benchmark_pump_efficiency.py "${ARGS[@]}"
} 2>&1 | tee "${LOG_PATH}"

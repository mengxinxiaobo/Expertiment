...
推荐参数：
    hidden=48, kernel_size=15
    参数量约 2,529，小于 PPLAD 2,561，容量尽量接近 PPLAD 但仍不超过它。

放置位置：
    /mnt/c/Users/DING/Desktop/Experiment/CODE/scripts/benchmarks/run_ad_asca_microfast_distill.py

推荐运行：
    CUDA_VISIBLE_DEVICES=0 python -u scripts/benchmarks/run_ad_asca_microfast_distill.py \
...
def build_solver_config(args: argparse.Namespace, teacher_kwargs: dict) -> dict:
    return {
        ...
        "model_save_path": str(ROOT / "checkpoints" / f"{args.dataset}_ASCA_MICROFAST" / "TEACHER"),
        "result_path": str(ROOT / "results" / f"{args.dataset}_ASCA_MICROFAST" / "TEACHER"),
...
def write_outputs(metrics: dict, out_dir: Path) -> None:
    ...
    lines = []
    lines.append(f"# ASCA-AD-MicroFast Distillation on {metrics.get('dataset', 'DATASET')}\n")

    lines.append("## Parameter Budget\n")
    lines.append(f"- PPLAD parameter budget: `{PPLAD_PARAM_BUDGET}`")
    lines.append(f"- Current parameter budget: `{metrics.get('param_budget', 'N/A')}`")
    ...
    lines.append(f"- Under current budget: `{metrics.get('under_current_budget', 'N/A')}`")
    lines.append("")
    ...
    lines.append("- 若要严格证明比 PPLAD 参数更少，请设置 `--param-budget 2561`。")
    lines.append("- 若允许略高于 PPLAD，但仍远低于 iTransformer，可使用默认 `--param-budget 10000`。")
    lines.append("")
...
def main() -> None:
    ...
    parser.add_argument(
        "--param-budget",
        type=int,
        default=DEFAULT_PARAM_BUDGET,
        help="参数量上限。默认 10000；若要严格小于 PPLAD，可设为 2561。",
    )
    ...
    if params["trainable_params"] > args.param_budget:
        raise RuntimeError(
            f"MicroFast 参数量 {params['trainable_params']} 超过当前参数预算 {args.param_budget}。"
            "请降低 --hidden / --kernel-size，或者提高 --param-budget。"
        )
    ...
    metrics = {
        ...
        "param_budget": int(args.param_budget),
        "under_pplad_budget": count_params(save_model)["trainable_params"] < PPLAD_PARAM_BUDGET,
        "under_current_budget": count_params(save_model)["trainable_params"] <= int(args.param_budget),
        "latency": {
            "batch_1": b1,
            "batch_n": bn,
            "full_test": ft,

#!/usr/bin/env python3
"""SKAB binding for the shared TranAD inference-efficiency benchmark."""
...
RESULT_ROOT = ROOT / "results" / "SKAB_TRANAD_RESULTS"
...
def configure() -> None:
    ...
    if not detection_protocol_path.is_file():
        raise FileNotFoundError(
            "Run the formal SKAB TranAD detection experiment before efficiency"
        )
    ...
    benchmark.EXPECTED_TEST_POINTS = 5710
    benchmark.DATASET_NAME = "SKAB"
    benchmark.WINDOW = 10
    benchmark.INPUT_C = 8
    benchmark.EXPECTED_PARAMETERS = 7208
    benchmark.TranADPSMDataAdapter = TranADSKABDataAdapter
    benchmark.load_tranad_psm_config = load_tranad_skab_config

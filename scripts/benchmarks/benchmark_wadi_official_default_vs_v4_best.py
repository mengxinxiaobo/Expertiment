...
"""
WADI 上 Original PPLAD 官方仓库默认配置与 V4 best 的统一轻量化基准。

公平性
------
- 轻量化测试统一使用 win_size=60、batch_size=128；
- Original：local_size=3、global_size=20、d_model=128；
- V4：local lags=[1..8]/Top-k=2，
      global lags=[12,16,20,24,28,32,40,48]/Top-k=4；
- 两个模型使用相同测试数据、设备、预热次数和重复次数；
- WADI 数据文件兼容 WaDi/WADI/wadi 三种前缀。

说明
----
官方仓库没有独立 scripts/WADI.sh。Original 检测实验采用官方
main.py 中的仓库默认参数，并替换为 WADI 的实际通道数。
"""
...
_WADI_BENCH_CACHE: Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray]] = {}
...
def normalize_wadi_labels(labels: np.ndarray) -> np.ndarray:
    ...
    raise ValueError(f"未知 WADI 标签值：{sorted(unique)}")
...
def install_flexible_wadi_loader(data_loader_module):
    ...
    class FlexibleWaDiSegLoader:
        def __init__(
            ...
            if key not in _WADI_BENCH_CACHE:
                ...
                _WADI_BENCH_CACHE[key] = (train, test, labels)

            train, test, labels = _WADI_BENCH_CACHE[key]
            self.train = train
            self.test = test
            self.val = test
...
def load_v4_checkpoint(model: V4BestCore, checkpoint: Path, device: torch.device) -> None:
    if not checkpoint.exists():
        raise FileNotFoundError(
            ...
            "请先运行 run_wadi_official_default_vs_v4_best.py --model v4。"
        )
...
def get_test_loader(
    ...
    try:
        import data_factory.data_loader as data_loader_module
    except ImportError as exc:
        raise RuntimeError(
            "无法导入 data_factory.data_loader。"
        ) from exc

    flexible_loader = install_flexible_wadi_loader(data_loader_module)
    return flexible_loader(
        137,
        str(dataset_root / "WADI"),
        batch_size=batch_size,
        win_size=win_size,
        mode="thre",
        dataset="WADI",
...
def parse_detection_log(model_name: str, root: Path) -> Dict[str, Any]:
    ...
    if model_name == "original":
        path = root / "results/WADI_OFFICIAL_DEFAULT_VS_V4_BEST/original_official_metrics.json"
        if not path.exists():
            return result
    ...
    path = root / "results/WADI_OFFICIAL_DEFAULT_VS_V4_BEST/v4_best_run.json"
    if not path.exists():
        return result
    data = json.loads(path.read_text(encoding="utf-8"))
    best = data.get("best", {})
...
def benchmark_one(args: argparse.Namespace) -> None:
    ...
    paths = resolve_wadi_files(dataset_root / "WADI")
    ...
    try:
        ...
        raise RuntimeError("WADI 测试集没有可用 batch。") from exc
...
def aggregate(args: argparse.Namespace) -> None:
    ...
    summary_lines = [
        "# WADI：Original PPLAD（官方仓库默认配置）vs V4（各自最优结果）",
        "",
        "## 检测精度",
        "",
        ...
        "",
        "说明：检测精度部分，Original 使用官方仓库 main.py 默认配置并应用于 WADI，V4 使用自身最佳配置及预定义阈值网格中的最高 PA-F1。轻量化部分为受控协议：两种模型统一使用 win_size=60、batch_size=128、相同硬件和相同测试流程。CPU RSS 容易受 Python 分配器影响，应以多次独立进程均值为正式结果。",
    ]
    summary = "\n".join(summary_lines)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/WADI_OFFICIAL_DEFAULT_VS_V4_BEST/BENCHMARK",
    )
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "WADI_adaptive_anchor_v4_l1-2-3-4-5-6-7-8_"

...
"""
SMD 上 Original PPLAD 与 V4 best 的统一轻量化基准。
...
------
- 数据集：SMD，38 通道；
- win_size=105，batch_size=128；
- Original：local_size=7，global_size=11，d_model=128；
...
def load_v4_checkpoint(model: V4BestCore, checkpoint: Path, device: torch.device) -> None:
    if not checkpoint.exists():
        raise FileNotFoundError(
            ...
            "请先运行 run_smd_official_vs_v4_best.py --model v4。"
        )
...
def parse_detection_log(model_name: str, root: Path) -> Dict[str, Any]:
    ...
    if model_name == "original":
        path = root / "results/SMD_OFFICIAL_VS_V4_BEST/original_official_metrics.json"
        if not path.exists():
            return result
    ...
    path = root / "results/SMD_OFFICIAL_VS_V4_BEST/v4_best_run.json"
    if not path.exists():
        return result
    data = json.loads(path.read_text(encoding="utf-8"))
    best = data.get("best", {})
...
def benchmark_one(args: argparse.Namespace) -> None:
    ...
    train_file = dataset_root / "SMD/SMD_train.npy"
    test_file = dataset_root / "SMD/SMD_test.npy"
    if not train_file.exists() or not test_file.exists():
        raise FileNotFoundError("找不到 dataset/SMD/SMD_train.npy 或 SMD_test.npy。")

    channels = int(np.load(train_file, mmap_mode="r").shape[1])
    if channels != 38:
        raise ValueError(f"SMD 应为 38 通道，实际为 {channels}。")
    ...
    try:
        ...
        raise RuntimeError("SMD 测试集没有可用 batch。") from exc
...
def aggregate(args: argparse.Namespace) -> None:
    ...
    summary_lines = [
        "# SMD：Original PPLAD（官方配置）vs V4（各自最优结果）",
        "",
        "## 检测精度",
        "",
        ...
        "",
        "说明：检测精度部分，Original 使用官方 SMD 配置，V4 使用自身最佳配置及预定义阈值网格中的最高 PA-F1。轻量化部分为受控协议：两种模型统一使用 win_size=105、batch_size=128、相同硬件和相同测试流程。CPU RSS 容易受 Python 分配器影响，应以多次独立进程均值为正式结果。",
    ]
    summary = "\n".join(summary_lines)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        default="results/SMD_OFFICIAL_VS_V4_BEST/BENCHMARK",
    )
    parser.add_argument("--device", choices=["auto", "cpu", "cuda"], default="auto")
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--win-size", type=int, default=105)

...
比较口径
--------
1. Original PPLAD：严格使用官方 scripts/MSL.sh 配置。
...
def verify_data(root: Path) -> int:
    data_dir = root / "dataset" / "MSL"
    paths = {
        "train": data_dir / "MSL_train.npy",
        "test": data_dir / "MSL_test.npy",
        "label": data_dir / "MSL_test_label.npy",
    }
    for path in paths.values():
        if not path.exists():
            raise FileNotFoundError(f"缺少 MSL 数据文件：{path}")
    ...
    if train.ndim != 2 or test.ndim != 2:
        raise ValueError(f"MSL 数据应为二维：train={train.shape}, test={test.shape}")
    if train.shape[1] != test.shape[1]:
        raise ValueError("MSL train/test 通道数不一致。")
    if len(test) != len(labels):
        raise ValueError(f"MSL test 与 label 长度不一致：{len(test)} vs {len(labels)}")

    channels = int(train.shape[1])
    if channels != 55:
        raise ValueError(f"MSL 官方配置应为 55 通道，实际为 {channels}。")

    print("MSL data check passed")
...
def original_config(root: Path, channels: int) -> dict:
    """官方 infogroup502/PPLAD scripts/MSL.sh 配置。"""
    return {
        "index": 137,
        "dataset": "MSL",
        "data_path": "MSL",
        ...
        "loss_fuc": "MSE",
        "anormly_ratio": 0.83,
        "model_save_path": str(root / "checkpoints" / "MSL_OFFICIAL_VS_V4_BEST" / "ORIGINAL"),
        "mode": "train",
        "rec_timeseries": True,
...
def v4_config(root: Path, channels: int, seed: int, epochs: int) -> dict:
    """
    当前 MSL 最佳结构 official_k6。
    阈值不固定在 config 中，训练完成后由网格搜索独立选择。
    """
    return {
        "dataset": "MSL",
        "data_path": "MSL",
        ...
        "num_epochs": epochs,
        "lr": 1e-3,
        "anormly_ratio": 0.83,  # 仅满足父类字段；最终结果由独立搜索确定。
        "index": 137,
        "mode": "train",
        "seed": seed,
        ...
        "similar": "MSE",
        "rec_timeseries": True,
        "model_save_path": str(root / "checkpoints" / "MSL_OFFICIAL_VS_V4_BEST" / "V4"),
        "result_path": str(root / "results" / "MSL_OFFICIAL_VS_V4_BEST" / "V4"),
        "use_gpu": torch.cuda.is_available(),
        "use_multi_gpu": False,
        "gpu": 0,
        "devices": "0",
...
def search_v4_best(
    ...
    train_scores, _ = collect_v4_scores(runner, runner.train_loader, include_labels=False)
    test_scores, gt = collect_v4_scores(runner, runner.thre_loader, include_labels=True)
    if gt is None:
        raise RuntimeError("未收集到 MSL 标签。")

    rows: List[Dict[str, object]] = []
    for mode in runner.score_modes:
        combined = np.concatenate([train_scores[mode], test_scores[mode]], axis=0)
        for ratio in ratios:
            threshold = float(np.percentile(combined, 100.0 - float(ratio)))
    ...
    # 首要目标 PA-F1；并列时依次选择 PA-Precision、PA-Recall、PA-Accuracy 更高者。
    best = max(
        rows,
        key=lambda row: (
            float(row["pa_f1"]),
    ...
    best_ratio = float(best["anormly_ratio"])
    ...
    result: Dict[str, object] = {
        ...
        "ratio_grid": {
            ...
            "count": int(len(list(ratios))) if not isinstance(ratios, np.ndarray) else int(ratios.size),
        },
        "best": best,
    ...
    print(f"anormly_ratio : {best_ratio}")
    print(f"threshold     : {best_threshold}")
    print(
        "PA Accuracy={:.4f}, Precision={:.4f}, Recall={:.4f}, F1={:.4f}, MCC={:.4f}".format(
...
def run_original(Solver, root: Path, channels: int) -> None:
    ...
    output_dir = root / "results" / "MSL_OFFICIAL_VS_V4_BEST"
    checkpoint_dir = Path(config["model_save_path"])
    output_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_dir.mkdir(parents=True, exist_ok=True)

    print("================ Original PPLAD / OFFICIAL MSL ================")
    ...
    checkpoint = checkpoint_dir / "MSL_original_official_state.pt"
    ...
    metrics = {
        ...
        "protocol": "official scripts/MSL.sh",
        "config": config,
...
def main() -> None:
    ...
    # 这里提前创建目录，避免 MSL 在指标已计算完成后因目录不存在而中断。

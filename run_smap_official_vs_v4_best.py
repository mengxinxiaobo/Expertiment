...
比较口径
--------
1. Original PPLAD：严格使用官方 scripts/SMAP.sh 配置。
...
def verify_data(root: Path) -> int:
    data_dir = root / "dataset" / "SMAP"
    paths = {
        "train": data_dir / "SMAP_train.npy",
        "test": data_dir / "SMAP_test.npy",
        "label": data_dir / "SMAP_test_label.npy",
    }
    for path in paths.values():
        if not path.exists():
            raise FileNotFoundError(f"缺少 SMAP 数据文件：{path}")
    ...
    if train.ndim != 2 or test.ndim != 2:
        raise ValueError(f"SMAP 数据应为二维：train={train.shape}, test={test.shape}")
    if train.shape[1] != test.shape[1]:
        raise ValueError("SMAP train/test 通道数不一致。")
    if len(test) != len(labels):
        raise ValueError(f"SMAP test 与 label 长度不一致：{len(test)} vs {len(labels)}")

    channels = int(train.shape[1])
    if channels != 25:
        raise ValueError(f"SMAP 官方配置应为 25 通道，实际为 {channels}。")

    print("SMAP data check passed")
...
def original_config(root: Path, channels: int) -> dict:
    """官方 infogroup502/PPLAD scripts/SMAP.sh 配置。"""
    return {
        "index": 137,
        "dataset": "SMAP",
        "data_path": "SMAP",
        ...
        "loss_fuc": "MSE",
        "anormly_ratio": 0.80,
        "model_save_path": str(root / "checkpoints" / "SMAP_OFFICIAL_VS_V4_BEST" / "ORIGINAL"),
        "mode": "train",
        "rec_timeseries": True,
...
def v4_config(root: Path, channels: int, seed: int, epochs: int) -> dict:
    ...
    return {
        ...
        "num_epochs": epochs,
        "lr": 1e-3,
        "anormly_ratio": 2.0,  # 仅满足父类字段；最终结果由独立搜索确定。
        "index": 137,
        "mode": "train",
        "seed": seed,
        ...
        "similar": "MSE",
        "rec_timeseries": True,
        "model_save_path": str(root / "checkpoints" / "SMAP_OFFICIAL_VS_V4_BEST" / "V4"),
        "result_path": str(root / "results" / "SMAP_OFFICIAL_VS_V4_BEST" / "V4"),
        "use_gpu": torch.cuda.is_available(),
        "use_multi_gpu": False,
        "gpu": 0,
        "devices": "0",
...
def search_v4_best(
    ...
    if gt is None:
        raise RuntimeError("未收集到 SMAP 标签。")
...
def run_original(Solver, root: Path, channels: int) -> None:
    ...
    output_dir = root / "results" / "SMAP_OFFICIAL_VS_V4_BEST"
    checkpoint_dir = Path(config["model_save_path"])
    output_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_dir.mkdir(parents=True, exist_ok=True)

    print("================ Original PPLAD / OFFICIAL SMAP ================")
    ...
    checkpoint = checkpoint_dir / "SMAP_original_official_state.pt"
    ...
    metrics = {
        ...
        "protocol": "official scripts/SMAP.sh",
        "config": config,
...
def main() -> None:
    ...
    parser.add_argument("--ratio-max", type=float, default=5.00)
    ...
    # 这里提前创建目录，避免 SMAP 在指标已计算完成后因目录不存在而中断。
